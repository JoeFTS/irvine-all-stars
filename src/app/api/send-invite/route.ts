...
<table cellpadding="0" cellspacing="0">
<tr><td style="padding:4px 12px 4px 0;vertical-align:top;color:#C1121F;font-size:14px;">&#9733;</td><td style="padding:4px 0;color:#4B5563;font-size:14px;line-height:1.5;">Upload required documents (birth certificate)</td></tr>
...
async function validateTeamDivision(
  ...
  if (!division) {
    return {
      ...
      response: NextResponse.json(
        { error: "team_id does not belong to the specified division" },
        { status: 400 }
      ),
    };
  }
  const { data: team, error } = await client
    ...
    .eq("id", teamId)
    .maybeSingle();
  if (error || !team || team.division !== division) {
    return {
      ok: false,
      response: NextResponse.json(
        { error: "team_id does not belong to the specified division" },
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (role === "coach" && Array.isArray(children) && children.length > 0) {
      // Validate team_id belongs to the specified division
      if (team_id) {
        const check = await validateTeamDivision(supabase, team_id, division);
    ...
    // Validate team_id belongs to the specified division (coach only)
